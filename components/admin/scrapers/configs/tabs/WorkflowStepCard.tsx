...
function getStepSummary(action: string, params: Record<string, unknown>): string {
  switch (action) {
    ...
    case 'conditional_click':
      return params.selector ? `Click if exists: ${String(params.selector)}` : 'Conditional click';
    case 'verify':
      return params.selector ? `Verify: ${String(params.selector)}` : 'Verify element';
    case 'scroll':
    ...
    case 'transform_value':
      return params.field ? `Transform: ${String(params.field)}` : 'Transform value';
    case 'execute_script':
...
// Nested component for extract_and_transform fields
function ExtractFieldsArray({ workflowIndex }: { workflowIndex: number }) {
  ...
  // Use useWatch to only re-render when this specific workflow's fields change
...
const WorkflowStepCard = React.memo(function WorkflowStepCard({
  index,
  control,
  remove,
  move,
  ...
  // Use useWatch to subscribe ONLY to this specific workflow step's data
  // This prevents re-renders when other workflow steps change
  const stepData = useWatch({
    control,
  ...
  const stepParams = stepData?.params || {};
          ...
          <div className="mt-4 pt-4 border-t space-y-4">
            <div className="flex items-center justify-between">
              ...
              <Button
                ...
                size="sm"
                onClick={() => {
                  const currentFields = (stepParams?.fields as Array<Record<string, unknown>>) || [];
            ...
            <div className="p-4 bg-amber-50 border border-amber-200 rounded-lg">
              ...
              <Button
                ...
                onClick={() => {
                  ...
                  const oldFields = stepParams?.fields as string[] | undefined;
                  const oldSelectorIds = stepParams?.selector_ids as string[] | undefined;
