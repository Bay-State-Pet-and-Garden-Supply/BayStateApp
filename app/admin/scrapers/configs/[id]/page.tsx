...
export default async function ScraperConfigPage({ params }: PageProps) {
  ...
  // Sequential fetch required: version_id from configRow is needed to fetch version
  // Cannot parallelize due to hard dependency on configRow.current_version_id
  // Cannot use join due to RLS constraints
  const supabase = await createClient();
